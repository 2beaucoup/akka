...
class GzipSpec extends CoderSpec {
  override protected def Coder = Gzip
  override protected def newDecodedInputStream(underlying: InputStream) = new GZIPInputStream(underlying)
  override protected def newEncodedOutputStream(underlying: OutputStream) = new GZIPOutputStream(underlying)

  override def extraTests(): Unit = {
    "decode concatenated compressions" in {
      pending // FIXME: unbreak
      ourDecode(Seq(ourEncodeString("Hello, "), ourEncodeString("dear "), ourEncodeString("User!")).join) should readAs("Hello, dear User!")
    }
