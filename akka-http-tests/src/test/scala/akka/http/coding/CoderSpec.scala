...
abstract class CoderSpec extends WordSpec with CodecSpecSupport with Inspectors {
  ...
  s"The ${Coder.encoding.value} codec" should {
    ...
    "properly round-trip encode/decode an HttpRequest" in {
      ...
      Coder.decode(Coder.encode(request), maxBytesPerChunk).toStrict(1.second).awaitResult(1.second) should equal(request)
    }
    ...
    "not throw an error if a subsequent block is corrupt" in {
      ...
      ourDecode(Seq(ourEncodeString("Hello,"), ourEncodeString(" dear "), corruptContent).join) should readAs("Hello, dear ")
    }
    "decompress in very small chunks" in {
      val compressed = ourEncodeString("Hello")
    ...
    */
    ...
    "works for chunked compressed data of sizes just above 1024" in {
      ...
      val compressed = Source.single(inputBytes).via(Coder.encoder).runWith(Sink.head()).awaitResult(1.second) //comp.compressAndFinish(inputBytes)
  ...
  def ourEncodeString(s: String) = Coder.encodeString(s)
  def ourEncode(bytes: ByteString): ByteString = Coder.encodeBytes(bytes)
  def ourDecode(bytes: ByteString): ByteString = Coder.decodeBytes(bytes)

  lazy val corruptContent = {
    val content = ourEncodeString(largeText).toArray
  ...
  def decodeChunks(input: Source[ByteString, _]): ByteString =
    input.via(Coder.decoder(65536)).join.awaitResult(3.seconds)

  def decodeFromIterator(iterator: () ⇒ Iterator[ByteString]): ByteString =
    Await.result(Source(iterator).via(Coder.decoder(65536)).join, 3.seconds)
}
